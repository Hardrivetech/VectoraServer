...
int load_chunk_nbt_at(const world_info_t *info,
    ...
    if (info == NULL || out_chunk_nbt == NULL || out_chunk_nbt_len == NULL) {
        return 0;
    }
    ...
    *out_chunk_nbt_len = 0;
    ...
    region_x = floor_div32(chunk_x, 32);
    region_z = floor_div32(chunk_z, 32);
...

    {
        ...
        if (compression_type == 1 || compression_type == 2) {
            if (!inflate_buffer_auto(chunk_payload, chunk_payload_len, out_chunk_nbt, out_chunk_nbt_len)) {
                free(region_file);
                return 0;
            }
        } else if (compression_type == 3) {
            *out_chunk_nbt = (uint8_t *)malloc(chunk_payload_len);
            if (*out_chunk_nbt == NULL) {
                free(region_file);
                return 0;
            }
            memcpy(*out_chunk_nbt, chunk_payload, chunk_payload_len);
            *out_chunk_nbt_len = chunk_payload_len;
        } else {
            free(region_file);
            return 0;
        }
...
static int load_spawn_chunk(world_info_t *info) {
    if (!load_chunk_nbt_at(info,
                           info->spawn_chunk_x,
                           info->spawn_chunk_z,
                           &info->spawn_chunk_nbt,
                           &info->spawn_chunk_nbt_len)) {
        return 0;
    }
